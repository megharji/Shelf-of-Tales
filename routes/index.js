...
function Books(){ // We substituted this in a function so that we don't have to rewrite the code We can simply call it for any number of times
  return JSON.parse(fs.readFileSync("public/book.json" , "utf-8")) // parse work is to covert string to json and readFileSync will read the file
}

function WriteBooks(books){ // We substituted this in a function so that we don't have to rewrite the code We can simply call it for any number of times
  fs.writeFileSync("public/book.json", JSON.stringify(books)) //stringify(x) This will convert X that is in Json form into string and writeFileSyncWill create a file again instant of append(or to set the data)
}

router.get('/register', function(req, res, next) {
     //We will get names of the country stored in a variable country names
    res.render('register', {
    countryNames : countryNames, // 1st is the Name we want to asign to the 2nd one
...
router.get('/delete/:index', function(req, res, next) {
  let x = Books()
  x.splice(req.params.index, 1)                                       // Splice will remove the title that we have got from the index
  WriteBooks(x)                                               // This will Create file with the update 
  res.redirect("/books")
});

router.get('/update/:index', function(req, res, next) {  // This is too create file update and get data through index and substitute in book and use it to get data  //The time we will click on update of that particular book we will get the index of that book in the router
  const books = Books()
  const book = books[req.params.index];  //Thtough req.params.index we will get the index from the router and that will be saved in book This is written to gather inform of the already written book
  res.render('update', {
...
router.post('/update/:index', function(req, res, next) {  //This is to update data which is written by the user again on the all over the old data
  let x = Books()
  x[req.params.index] = req.body                                    // Through req.body We will get the data which is updated and we will just substitute or update in that index
  WriteBooks(x)                                                // This will Create file with the update 
  res.redirect("/books")
});
